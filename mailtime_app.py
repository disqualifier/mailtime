...
class MailClient(QMainWindow):
    ...
    def __init__(self):
        ...
        # Check for updates on startup (delayed to avoid blocking UI)
    ...
    def _setup_accounts_panel(self):
        ...
        # Add email search bar with debouncing
        ...
        # Connect with debouncing (wait 300ms after user stops typing)
    ...
    def update_tab_status(self, tab, status):
        ...
        if index >= 0:
            # Find the correct account by matching email address from the tab
    ...
    def _check_for_updates(self):
        ...
        try:
            # For now, we'll use a placeholder repo URL - replace with actual repo when available
            repo_url = "https://api.github.com/repos/dsql/mailtime/releases/latest"  # Replace with actual repo
    ...
    def _do_email_search(self):
        ...
        try:
            ...
            # Filter the accounts list based on search text
            for i in range(self.accounts_list.count()):
                item = self.accounts_list.item(i)
                if item:
                    # Get account text (which includes email and status info)
                    account_text = item.text().lower()

                    # Show/hide based on whether search text matches
    ...
    def _filter_current_tab_emails(self, search_text: str):
        ...
        # This method is now replaced by the debounced search
